...
def setStatus(status, job_id):
    ...
    cursor = db.cursor()
    cursor.execute('UPDATE "_WORKER2" SET STATUS = \'' + str(status) + '\' WHERE JOB_ID = ' + str(job_id))
    db.commit()
    if cursor:
        cursor.close()
    if db:
...
def fetchJob():
    ...
    cursor = db.cursor()
    cursor.execute('SELECT JOB_ID, utl_raw.cast_to_varchar2(JOB), STATUS FROM "_WORKER2" WHERE STATUS = \'N\' AND ROWNUM = 1')
    ...
    if result:
        ...
        updateExeTable(result[0][0], '_WORKER2', status, start_time, end_time, res)
    if cursor:
        cursor.close()
    if db:
        db.close()
